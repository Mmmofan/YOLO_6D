...
def data_augmentation(img, shape, jitter, hue, saturation, exposure):
    ...
    dw =int(ow*jitter)  # w方向最大移动百分比
    dh =int(oh*jitter)  # h方向最大移动百分比

    pleft  = random.randint(-dw, dw)  # 左移百分比
    pright = random.randint(-dw, dw)  # 右移百分比
    ptop   = random.randint(-dh, dh)  # 上移百分比
    pbot   = random.randint(-dh, dh)  # 下移百分比

    swidth =  ow - pleft - pright
    sheight = oh - ptop - pbot

    sx = float(swidth)  / ow  # w方向偏移比例
    sy = float(sheight) / oh  # h方向偏移比例
