...
class yolo_v2(object):
    ...
    def build_networks(self, inputs):
        ...
        net = self.conv_layer(net, [1, 1, 1024, 1024], batch_norm=False, name='30_conv')
        net = self.flatten(net, name='flatten')
        net = self.fc(net, [net.get_shape().as_list()[1], 15], name='full_connect')
    ...
    def fc(self, inputs, shape, name='full_connect'):
        weight = tf.Variable(tf.truncated_normal(shape, stddev=0.1), name='weight')
        biases = tf.Variable(tf.constant(0.1, shape=[shape[1]]), name='biases')
        fc = tf.nn.relu(tf.nn.bias_add(tf.matmul(inputs, weight), biases))
        return fc
